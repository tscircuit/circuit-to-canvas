...
export function drawPcbCopperText(params: DrawPcbCopperTextParams): void {
  ...
  if (text.is_knockout) {
    ...
    // startY is the center (middle baseline), text box extends from (center - height/2) to (center + height/2)
    const textBoxTop = startY - layout.height / 2 - layout.strokeWidth / 2
    const textBoxBottom = startY + layout.height / 2 + layout.strokeWidth / 2
